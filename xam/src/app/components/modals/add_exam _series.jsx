...
export default function Add_exams_series({ open, setOpen, exam }) {
  const [name, setName] = useState("");
  const [exam_id, setExamID] = useState(exam[0].id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("/api/exam_series", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(examSeriesData),
      });

      if (res.ok) {
        console.log("Exam Series created successfully");
